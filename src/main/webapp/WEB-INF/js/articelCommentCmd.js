...
var loadCommentFunc = function () {
    ...
    loadCommentDataAjax = $.ajax({
        method: 'POST',
        url: 'http://' + IP + ':' + PORT_PATH + '/article/getCommentAndResponseData',
        ...
        success: function (data) {
            ...
            if (data !== FAILURE) {
                ...
                if (fromCommentTableDataArr[0].length !== 0)
                    $('#show_comment_number_p').html(fromCommentTableDataArr.length + '条评论:');
            }
        }
    }).fail(function () {
        showOrHiddenLoadingDiv(false);
        ...
        fromCommentTableDataArr.forEach(function (commentData, index) {
            ...
            if (id !== rId) {
                ...
                $('#show_comment_area_div').append($(' <div class="part_of_comment_div">\n' +
                    '        <img class="commenter_img" id="commenter_img_' + index + '" src="/imgs/comment_icon.png"/>\n' +
                    '        <span class="commenter_span"  id="commenter_id_' + index + '"></span>\n' +
                    '        <span class="comment_datetime_span" id="comment_datetime_span_' + index + '"></span>\n' +
                    '        <div class="comment_area previous_comment_div">\n' +
                    '            <span class="commenter_span" id="previous_commenter_id_span_' + index + '"></span><br>\n' +
                    '            <p class="comment_area" id="previous_comment_content_p_' + index + '"></p>\n' +
                    '        </div>\n' +
                    '        <p class="comment_area" id="comment_content_p_' + index + '">\n' +
                    '            </p>\n' +
                    '        <img class="comment_reply_img" id="comment_reply_img_' + index + '" src="/imgs/reply_icon.png"/>\n' +
                    '    </div>'));
                //绑定相应事件和相关赋值操作
                let cIdAppointCommentData = fromCommentTableDataArr[fromCommentTableDataArr.length - cId];
                let endIndex = Math.ceil((6 / 17) * cIdAppointCommentData[1].length);
                if (endIndex > 6)
                    endIndex = 6;
                $('#' + 'previous_commenter_id_span_' + index).html(cIdAppointCommentData[1].substring(0, endIndex));
                $('#' + 'previous_comment_content_p_' + index).html(cIdAppointCommentData[2]);

                analyzeDataAndCreateElementEventAndInsertDataNoPreviousCommentDivFunc(commentData, index);
...
var analyzeDataAndCreateElementNoPreviousCommentDivFunc = function (commentData, index) {
    $('#show_comment_area_div').append($(' <div class="part_of_comment_div">\n' +
        '        <img class="commenter_img" id="commenter_img_' + index + '" src="/imgs/comment_icon.png"/>\n' +
        '        <span class="commenter_span"  id="commenter_id_' + index + '"></span>\n' +
        '        <span class="comment_datetime_span" id="comment_datetime_span_' + index + '"></span>\n' +
...
var analyzeDataAndCreateElementEventAndInsertDataNoPreviousCommentDivFunc = function (commentData, index) {
    //设置commenterId
    ...
    publish_span.onclick = function () {
        ...
        if (reg.test(email)) {
            ...
            if ($('#wanna_responsed_comment_info_div').css('display') === 'none') {
                url = 'http://' + IP + ':' + PORT_PATH + '/article/insertCommentContentToAppointTable';
                data = {
                    tableName: tableName,
                    commenterId: email,
                ...
                url = 'http://' + IP + ':' + PORT_PATH + '/article/insertResponseAndCommentContentToAppointTable';
                data = {
                    tableName: tableName,
